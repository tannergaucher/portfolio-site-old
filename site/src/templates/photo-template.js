      ...
      <article className="container res-padding center page">
        <figure className="figure">
          ...
          <figcaption className="figcaption padding">
            <time className="text--sm">{formatedDateTime}</time>{" "}
            <span className="text--md">{data.sanityMyImage.caption}</span>
          </figcaption>
        </figure>
        <nav className="padding">
          {next && (
            <Link className="nav-link" to={`/photo/${next.slug.current}`}>
              <h4 className="text--sm">Next</h4>
            </Link>
          )}
          {previous && (
            <Link className="nav-link" to={`/photo/${previous.slug.current}`}>
              <h4 className="text--sm">Previous</h4>
            </Link>
          )}
